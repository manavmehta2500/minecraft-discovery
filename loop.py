...
Modes:
  python loop.py            # infinite loop (Railway/Fly/VPS)
  python loop.py --once     # one cycle then exit (GitHub Actions / cron)
"""
import argparse
import logging
import signal
...
def _handle_signal(signum, frame):
    global _shutdown
    log.info("Received signal %s — exiting after current source", signum)
    _shutdown = True
...
def _run_cycle():
    ...
    for name, fn in sources:
        ...
        if published_total >= MAX_ITEMS:
            log.info("Reached MAX_ITEMS=%d — stopping early", MAX_ITEMS)
            break
...
def main():
    ...
    log.info(
        "Starting (mode=%s, max_items=%d, sleep=%ds, dry_run=%s, once=%s)",
    ...
    if args.once:
        ...
        log.info("One-shot complete: published=%d, elapsed=%.1fs",
                 published, time.time() - start)
        return 0

    cycle = 0
    while not _shutdown:
        cycle += 1
        log.info("========== Discovery cycle #%d ==========", cycle)
        start = time.time()
        try:
            published = _run_cycle()
        except Exception as e:
        ...
        log.info("Cycle #%d done: published=%d, elapsed=%.1fs. Sleeping %ds.",
